...
The OSR mechanism described in Chapter~\ref{ch:design} sits at the simpler end of the spectrum that D'Elia and Demetrescu describe. The transfer points are restricted to outermost-loop back-edges, the live state at each transfer point is exactly the set of wasm-level locals (a syntactically delimited set rather than a register-and-spill location set), and the continuation is synthesized as a fresh wasm function whose entry point is the loop header. The simplification is made possible by WebAssembly's structured control flow and its explicit local-variable model. The price is a conservative filter that rejects loops whose surrounding control flow cannot be safely re-entered mid-execution; the rejection is discussed in Section~\ref{sec:design:osr:synthesis} and the residual scope limitation in Chapter~\ref{ch:conclusion}.

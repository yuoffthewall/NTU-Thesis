...
本論文提出一套適用於 WasmEdge 的分層編譯架構，目的在於將啟動延遲與最終程式碼品質解耦。本架構由三個元件組成。第一層為 baseline JIT 編譯層 (Tier-1)，整合 dstogov/ir SSA 函式庫，於模組載入時以低成本將所有函式編譯為原生程式碼，立即提供執行能力。第二層為優化編譯層 (Tier-2)，重用 WasmEdge 既有的 LLVM 前端，透過合成 mini-module、三類 ABI thunk 橋接機制，以及納入靜態熱點呼叫對象的批次組合策略，選擇性地重新編譯熱點函式。第三項為在堆疊替換 (On-Stack Replacement, OSR) 機制，於最外層迴圈反向邊插入監測點，將正在執行中的 tier-1 框架於迴圈執行過程中遷移至 tier-2，解決函式入口替換在「單次呼叫者」場景下無法加速的問題。

本架構於 WasmEdge 中實作，並以 sightglass 測試套件進行評估---該套件經強化以延長工作負載執行時間，使優化層獲得可測量的提升後執行窗口。Sightglass 涵蓋合成微基準測試、密碼學原語、解析與序列化工作負載，以及若干大型真實應用模組，包括 SpiderMonkey JavaScript 引擎、SQLite 資料庫、TinyGo 編譯之 Go 程式，以及 ONNX-runtime 影像分類器。在 39 個分析測試項目上，本架構將編譯時間相較於完整模組 LLVM-JIT 縮短了幾何平均 11.1 倍，同時於穩態吞吐量上仍保留了約百分之八十六的 LLVM-JIT 表現。端到端執行時間因此平均縮減 1.73 倍，於最大型工作負載上更展現出數量級的改進：\texttt{tinygo-json} 達 21.9 倍、ONNX 分類器達 14.1 倍、三個 SpiderMonkey 測試項目皆達約 10 倍。完整設定相較於 tier-1 baseline 取得幾何平均 1.11 倍的加速；OSR 機制亦於單次呼叫者場景中彌補殘餘差距---將 \texttt{shootout-ctype} 由 0.37 倍提升至 0.88 倍、\texttt{blind-sig} 由 0.40 倍提升至 0.80 倍（相對於 LLVM-JIT）。
...
The architecture is implemented in WasmEdge and evaluated on the sightglass benchmark suite, strengthened with longer-running workloads to give the optimizing tier a measurable post-promotion window. Sightglass spans synthetic micro-benchmarks, cryptographic primitives, parsing and serialization workloads, and several large real-world modules including the SpiderMonkey JavaScript engine, SQLite, TinyGo-compiled Go programs, and ONNX-runtime image classifiers. Across 39 analyzed kernels, the tiered architecture cuts compile time by a geometric-mean factor of 11.1$\times$ over whole-module LLVM-JIT while preserving approximately 86\% of LLVM-JIT's steady-state throughput. End-to-end wall-clock latency therefore improves by 1.73$\times$ on average, with order-of-magnitude wins on the largest workloads: 21.9$\times$ on \texttt{tinygo-json}, 14.1$\times$ on the ONNX classifier, and approximately 10$\times$ on each of the three SpiderMonkey kernels. The full configuration beats the tier-1 baseline by a geometric-mean 1.11$\times$, and on-stack replacement closes the residual gap on one-shot-caller workloads---raising \texttt{shootout-ctype} from 0.37$\times$ to 0.88$\times$ and \texttt{blind-sig} from 0.40$\times$ to 0.80$\times$ relative to LLVM-JIT.
